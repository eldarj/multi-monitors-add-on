...
import { gettext as _ } from './extension.js';

// Derive extension path from this module's URL
const _extensionPath = GLib.filename_from_uri(import.meta.url)[0].replace(/\/[^/]*$/, '');

export var MultiMonitorsIndicator = GObject.registerClass(
class MultiMonitorsIndicator extends PanelMenu.Button {
    ...
    _icon_name(icon, iconName) {
        icon.set_gicon(Gio.icon_new_for_string(_extensionPath + '/icons/' + iconName + '.svg'));
    }
    ...
    _onPreferences() {
        const uuid = 'multi-monitors-add-on@eldarj';
